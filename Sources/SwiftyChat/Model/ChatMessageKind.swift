...
public enum ChatMessageKind: CustomStringConvertible {
    ...
    case text(Bool,Bool,String,[String]?,MessagePriorityLevel,ActionItemStatus?,[MessageReaction]?)
    
    /// An image message, from local(UIImage) or remote(URL).
    case image(Bool,Bool,ImageLoadingKind,MessagePriorityLevel,ActionItemStatus?,[MessageReaction]?)
    
    /// An image message, from local(UIImage) or remote(URL).
    case imageText(Bool,Bool,ImageLoadingKind, String,[String]?,MessagePriorityLevel,ActionItemStatus?,[MessageReaction]?)
    ...
    case video(Bool,Bool,VideoItem,MessagePriorityLevel,ActionItemStatus?,[MessageReaction]?)
    
    case videoText(Bool,Bool,VideoItem,String,[String]?,MessagePriorityLevel,ActionItemStatus?,[MessageReaction]?)
    ...
    case reply(Bool,Bool,any ReplyItem,[any ReplyItem],MessagePriorityLevel,ActionItemStatus?,[MessageReaction]?)
    
    case pdf(Bool,Bool,ImageLoadingKind,String,[String]?,URL,MessagePriorityLevel,ActionItemStatus?,[MessageReaction]?)
    
    case audio(Bool,Bool,URL,MessagePriorityLevel,ActionItemStatus?,[MessageReaction]?)
    
    
    public var description: String {
        switch self {
        case .image(let isUrgent, let isAttention, let imageLoadingType, _, _,let reactions):
        ...
        case .imageText(let isUrgent, let isAttention,let imageLoadingType, let text, _, _, _,let reactions):
            switch imageLoadingType {
            case .local(let localImage):
        ...
        case .text(let isUrgent, let isAttention,let text,let attentions, _, _,let reactions):
        ...
        case .video(let isUrgent, let isAttention,let videoItem, _, _,let reactions):
            return "MessageKind.video(url: \(videoItem.url))"
        case .loading:
            return "MessageKind.loading"
        case .systemMessage(let message):
            return "MessageKind.systemMessage \(message)"
        case .videoText(let isUrgent, let isAttention,let videoItem,let text,let attentions, _, _,let reactions):
            return "MessageKind.video(url: \(videoItem.url) text \(text) tag \(attentions)"
        case . reply(let isUrgent, let isAttention,let reply, let replies, _, _,let reactions):
            return "MessageKind.reply reply \(reply) and replies \(replies)"
        
        case . pdf(let isUrgent, let isAttention,let image, let text, let attentions,let pdfUrl, _, _,let reactions):
            switch image {
            case .local(let localImage):
        ...
        case .audio(let isUrgent, let isAttention,let url, _ , _,let reactions):
